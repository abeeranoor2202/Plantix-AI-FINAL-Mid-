@php
    ...
    $navItems = [
        ...
        ['label' => 'Products', 'icon' => 'mdi-cart-outline', 'active' => Request::is('admin/products*'), 'route' => route('admin.products.index'), 'ability' => 'admin.products'],
        ['label' => 'Categories', 'icon' => 'mdi-shape-outline', 'active' => Request::is('admin/categories*'), 'route' => route('admin.categories'), 'ability' => 'admin.category'],
    ...
    $navItems = array_values(array_filter($navItems, function (array $item) {
        if (! isset($item['ability']) || ! Auth::guard('admin')->check()) {
            return true;
        }

        return Auth::guard('admin')->user()->can($item['ability']);
    }));
