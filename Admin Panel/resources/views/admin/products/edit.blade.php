...
$(document).ready(function () {
    ...
    var currentValues = @json(($product->attributes ?? collect())->mapWithKeys(function($attr) {
    ...
    function renderAttributeFields(categoryId) {
        ...
        list.forEach(function(attr) {
            ...
            var existing = currentValues[attr.id] || '';

            if (attr.type === 'multi-select') {
                var selected = normalizeMulti(existing);
                ...
                (attr.values || []).forEach(function(value) {
                    var isSelected = selected.includes(String(value)) ? 'selected' : '';
            ...
            if (attr.type === 'select') {
                ...
                (attr.values || []).forEach(function(value) {
                    var isSelected = String(existing) === String(value) ? 'selected' : '';
                    html += '<option value="' + esc(value) + '" ' + isSelected + '>' + esc(value) + '</option>';
                });
                html += '</select></div>';
                return;
            ...
            html += '<div class="col-md-6">'
                + '<label class="agri-label">' + esc(attr.name) + requiredMark + unitHint + '</label>'
                + '<input type="' + inputType + '" name="' + fieldName + '" class="form-agri" value="' + esc(existing) + '" ' + required + '>'
                + '</div>';
        });
