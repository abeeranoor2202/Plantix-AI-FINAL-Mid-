...
<div class="container-fluid" style="padding-top: 24px; padding-bottom: 40px;">
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 32px;">
        <div>
            ...
            <h1 style="font-size: 28px; font-weight: 700; color: var(--agri-primary-dark); margin: 0;">Crop Recommendations</h1>
            <p style="color: var(--agri-text-muted); margin: 4px 0 0 0;">Recent AI-driven crop advice records with soil source references.</p>
        </div>
    ...
    <div class="card-agri" style="padding: 0; overflow: hidden;">
        <div class="card-header bg-white border-bottom-0 pt-4 pb-3 px-4 d-flex justify-content-between align-items-center">
            <h4 class="mb-0 fw-bold text-dark" style="font-size: 18px;">Recommendation List</h4>
            ...
            <table class="table mb-0" style="vertical-align: middle;">
                <thead style="background: var(--agri-bg);">
                    <tr>
                        ...
                        <th style="padding: 16px 24px; font-size: 12px; font-weight: 600; color: var(--agri-text-muted); text-transform: uppercase; border: none;">Top Crop</th>
                        <th style="padding: 16px 24px; font-size: 12px; font-weight: 600; color: var(--agri-text-muted); text-transform: uppercase; border: none;">Crops Count</th>
                        <th style="padding: 16px 24px; font-size: 12px; font-weight: 600; color: var(--agri-text-muted); text-transform: uppercase; border: none;">Soil Source</th>
                ...
                <tbody>
                    @forelse($recommendations as $rec)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <span class="badge rounded-pill bg-success">{{ $rec->top_crop }}</span>
                            </td>
                            <td class="px-4 py-3">{{ count($rec->recommended_crops ?? []) }}</td>
                            <td class="px-4 py-3">{{ $rec->soil_test_id ? 'Lab Test #'.$rec->soil_test_id : 'Manual Input' }}</td>
                            <td class="px-4 py-3">{{ $rec->created_at->format('d M Y') }}</td>
                            <td class="px-4 py-3">
                                <div class="text-end" style="display: flex; justify-content: flex-end; gap: 8px;">
                                    <a href="{{ route('admin.ai.crop-recommendations.show', $rec->id) }}" class="btn-agri" style="padding: 8px; background: var(--agri-bg); color: #2563eb; border-radius: 999px;" title="View"><i class="fas fa-eye"></i></a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="text-center py-5" style="color: var(--agri-text-muted);">No recommendations yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
